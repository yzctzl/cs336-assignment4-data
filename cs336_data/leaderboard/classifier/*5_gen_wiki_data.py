...
def process_single_record(args):
    """
    Process a single WARC record with 15-second timeout.
    ...
    try:
        ...
        text = extract_text_from_html_bytes(content, True)
        ...
        signal.alarm(0)
        print("\n[TIMEOUT] Skipped slow record after 15s", file=sys.stderr, flush=True)
        return None
    except Exception:
        signal.alarm(0)
        return None
...
def generate_fasttext_data(wiki_warc_path: str, num_workers=None):
    if num_workers is None:
        num_workers = cpu_count()
    ...
    wiki_stream = stream_filtered_warc(
        wiki_warc_path, "__label__hq", filter_func=gopher_filter, num_workers=num_workers
    )
    # The loop terminates when the *shorter* stream of VALID documents is exhausted.
    for i, wiki_line in enumerate(wiki_stream):
        yield wiki_line
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train", type=str, default="data/paloma/wiki/train.txt", help="Train file path")
    parser.add_argument("--valid", type=str, default="data/paloma/wiki/valid.txt", help="Valid file path")
    parser.add_argument("--test", type=str, default="data/paloma/wiki/test.txt", help="Test file path")
    ...
    with (
        ...
        for line in generate_fasttext_data(args.wiki, num_workers=args.workers):
            if count % 10 == 0:
                f_test.write(line + "\n")
            elif count % 10 == 9:
                f_valid.write(line + "\n")
...
model = fasttext.load_model("data/classifiers/paloma_vs_cc.ftz")
results = []

with open(f"{args.wiki}") as f:
    for line in f:
        line = line.strip()
        label, prob = model.predict(line)
        if label[0] == "__label__hq":
            results.append((prob[0], line))

results.sort(key=lambda x: x[0], reverse=True)
with open("wiki_refs_5w_top_score.txt", "w") as f:
    for _, text in results[:50000]:
        f.write(text + "\n")
